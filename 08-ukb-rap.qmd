...
1. You need to apply for access and note what fields are relevant for your work. When the UK Biobank team has reviewed it, you will be assigned an Application ID. 
1. You are limited to sharing projects within your own application ID. All billing information is attached to the application ID, rather than to an organization.
1. Data is *dispensed* when requested at project creation or refresh. This is both the pheno data (the `dataset`) and the Bulk, or file based data.
1. The participant identifiers (`eid`s) are different across applications because of the psuedonymization process.
1. There is a `Bulk/` File Folder that contains many of the files you want to analyze.
...
The data dispensal process ...

- [Updating Dispensed Data](https://dnanexus.gitbook.io/uk-biobank-rap/getting-started/updating-dispensed-data)

## Pheno/Geno Data

Pheno and Processed Geno Data are stored within multiple Spark databases. These physical databases are accessed through a single entity called the *dataset*.
...
Each RStudio Project has its own library of packages. This is also handy in that you can work with multiple projects that each have their own version of a package.
