...
## The Four Filesystems (and how to avoid one)
...
Using Spark on DNAnexus effectively requires us to be familiar with 4 different file systems (@fig-full-graph):
...
2. When possible, use dxFUSE with glob wildcards (such as `*.tar.gz` or `chr*`) to directly stream files into HDFS (Spark Cluster Storage) from project storage by using `file:///mnt/project/` URLs (@fig-dxfuse-glob). For example, we could load a set of pVCF files with `hl.read_vcf("file:///mnt/project/data/pVCF/*.vcf.gz")`) 
...
5. More advanced usage - you can connect Spark to other S3 buckets using the `s3://` protocol. You will need to authenticate before connecting.
