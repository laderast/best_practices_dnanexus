...
You may be familar with *POSIX* (Portable Operating System Interface) file systems as a way to organize your data in folders. The *POSIX* standard basically defines the filesystem API so that they will be compatible with other file systems.
...
For example, the UKB RAP region (`eu-west-2`) is unique and only limited to projects within the scope of UKB.
...
The DNAnexus platform is only one example of an object-based filesystem. Other examples include Amazon S3 (Simple Storage Service), Microsoft Azure Blob Storage, and Google Cloud Storage. Why does the world run on object based filesystems? There are a lot of advantages. 
...
In [Bash for Bioinformatics](https://laderast.github.io/bash_for_bioinformatics/), we already discovered one way of working with files on the platform: [`dx find data`](https://laderast.github.io/bash_for_bioinformatics/05-batch-processing.html#sec-dx-find). This is our main tool for selecting files with metadata.
...
In addition to dxFUSE, there are a number of other POSIX/Object translation layers that try to make object-based file systems more POSIX-like, such as JuiceFS (for S3).
