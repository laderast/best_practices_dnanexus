...
You may be familar with *POSIX* (Portable Operating System Interface) filesystems as a way to organize your data. The main way to find/refer to a file is through its *path*. What does this mean?
...
Paths are the main way we organize files for POSIX filesystems. This information is external to a file. Most importantly, we use directories to organize and group files logically. For example, we might have our R script and the data it analyzes in the same directory. 
...
One important thing to remember: dxFUSE is only mounted once when you start JupyterLab. You will have to use the `dxfuse` command to remount it to see any files you've added to project storage after opening JupyterLab.
...
In addition to dxFUSE, there are a number of other POSIX/Object translation layers that try to make object-based file systems more POSIX-like, such as JuiceFS (for Redis/S3).
